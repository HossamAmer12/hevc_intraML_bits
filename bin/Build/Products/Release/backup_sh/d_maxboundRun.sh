...
#done
# >>>> This one for two QPref
#for ((qpRef = 20; qpRef<=$qpEnd;qpRef++))
#do
#  for ((qpRefTwo = 20; qpRefTwo<=$qpEndTwo;qpRefTwo++))
#  do
#       ./TAppEncoder -c cfg/$current_enc_cfg.cfg -c cfg/per-sequence/$current_cfg.cfg  \
#       -i "$sequences_dir"/$current_yuv.yuv -q ${Qp[$i]} -f $debugFrames \
#       -qpRef $qpRef -qpRefTwo $qpRefTwo                                 \
#       -b Gen/Seq-265/$current_bs.265 > Gen/Seq-TXT/$current_txt.txt
#       echo "Done QP Reference1 =" $qpRef "and Reference2 =" $qpRefTwo
#   done
#done
...
# Use that one for qp offset in the I-frame (old version)
# qpEnd=13 # offset -13
# for ((qpRef = 0; qpRef<=$qpEnd;qpRef++))
# do
# ## Run the qpOffset
#    ./TAppEncoder -c cfg/$current_enc_cfg.cfg -c cfg/per-sequence/$current_cfg.cfg  \
#        -i "$sequences_dir"/$current_yuv.yuv -q ${Qp[$i]} -qpRef $qpRef \
#        -b Gen/Seq-265/$current_bs.265 > Gen/Seq-TXT/$current_txt.txt
# #   ./TAppEncoder -c cfg/$current_enc_cfg.cfg -c cfg/per-sequence/$current_cfg.cfg  \
# #       -i "$sequences_dir"/$current_yuv.yuv -q ${Qp[$i]} -f $debugFrames -qpRef $qpRef \
# #       -b Gen/Seq-265/$current_bs.265 > Gen/Seq-TXT/$current_txt.txt
...
#./TAppEncoder -c cfg/$current_enc_cfg.cfg -c cfg/per-sequence/$current_cfg.cfg  \
#-i "$sequences_dir"/$current_yuv.yuv -q ${Qp[$i]} -qpRef $qpRef -qpFact $qpFact \
#-b Gen/Seq-265/$current_bs.265 > Gen/Seq-TXT/$current_txt.txt

# Run all for I frame QP offset
#./TAppEncoder -c cfg/$current_enc_cfg.cfg -c cfg/per-sequence/$current_cfg.cfg  -i "$sequences_dir"/$current_yuv.yuv -q ${Qp[$i]} -qpRef $qpRef -b Gen/Seq-265/$current_bs.265 > Gen/Seq-TXT/$current_txt.txt
